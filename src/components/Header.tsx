...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header className="bg-gradient-to-r from-emerald-950 via-emerald-900 to-teal-950 text-white shadow-xl border-b-4 border-amber-500 sticky top-0 z-40">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="hidden sm:inline-flex items-center text-[11px] font-semibold uppercase px-2 py-0.5 rounded bg-amber-500/20 text-amber-300 border border-amber-500/40">
                  Campus 32
                </span>
              </div>
              <p className="text-xs md:text-sm text-emerald-200/90 font-medium">
                Late Student Management & Punctuality System &bull; <span className="text-amber-300 font-semibold">بَابُ الْانْضِبَاطِ</span>
              </p>
            </div>
          </div>
      ...
      {/* Navigation Bar: Public Tabs on Left + Dedicated Management Tab AT THE SIDE */}
      <nav className="max-w-7xl mx-auto px-2 sm:px-4 flex items-center justify-between border-t border-emerald-800/80 overflow-x-auto scrollbar-none">
          ...
          {publicNavItems.map((item) => {
            ...
            return (
              <button
                ...
                className={`flex items-center gap-2 px-3.5 py-2.5 rounded-t-lg text-xs md:text-sm font-semibold transition-all duration-150 cursor-pointer relative whitespace-nowrap ${
                  isActive
        ...
        <div className="py-1 pl-3 min-w-max">
          <button
            id="tab-btn-management"
            onClick={() => handleTabClick('management')}
            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-xs md:text-sm font-bold transition-all duration-200 cursor-pointer shadow-md border ${
              isManagementActive
          ...
          >
            <div className="flex items-center gap-1.5">
              <FileEdit className={`w-4 h-4 ${isManagementActive ? 'text-emerald-950' : 'text-amber-300'}`} />
              <span>Management Panel</span>
            </div>

            {isAdminUnlocked ? (
              <span className="flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] font-bold bg-emerald-900/30 text-emerald-950">
                <ShieldCheck className="w-3 h-3 text-emerald-900" />
                <span>Admin</span>
              </span>
            ) : (
              <span className="flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] font-bold bg-amber-950/50 text-amber-300 border border-amber-500/30">
